...
func child() {
	log.Printf("Running: %v as %d, container uuid: %s\n", os.Args[3:], os.Getpid(), os.Args[2])
	image := os.Args[3]
	...
	if err := EnterNetns(fmt.Sprintf("netns_%s", os.Args[2])); err != nil {
		log.Fatalln("Error entering netns", err.Error())
	}
	...
	if strings.Contains(process, "/bin/") {
		...
		cmdString := process + strings.Join(process_args, " ")
		childPid, _, err = syscall.StartProcess("/bin/sh", []string{"sh", "-c", cmdString}, &config)
	}
	if err != nil {
...
func run() {
	...
	childUuid := fmt.Sprintf("%d", rand.Intn(100)+40000)
	if err := SetupNet(childUuid); err != nil {
		CleanupNet(childUuid)
		log.Fatalln("Failed to setup netns: ", err.Error())
	}

	defer CleanupNet(childUuid)
	...
	childPid, _, err := syscall.StartProcess("/proc/self/exe", append([]string{os.Args[0], "child", childUuid}, os.Args[2:]...), &config)
	if err != nil {
